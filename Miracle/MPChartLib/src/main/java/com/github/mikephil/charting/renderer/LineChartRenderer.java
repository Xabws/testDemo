...
public class LineChartRenderer extends LineRadarRenderer {
     ...
     * 画点到坐标轴之间的虚线
     */
    protected void drawFillLine(Canvas c, float startX, float endX, float startY, float endY, int color) {
        // draw a path because lines don't support dashing on lower android version
        // 画Y轴方向对应X轴坐标的网格虚线
       ...
       // mGridPaint.setAlpha(90);
        mGridPaint.setPathEffect(new DashPathEffect(new float[]{20, 8}, 0));
        c.drawLine(startX,  startY,endX, endY, mGridPaint);
    }
                ...
                for (int j = 0; j < positions.length; j += 2) {
                    ...
                    if (dataSet.isDrawValuesEnabled()) {
                        ...
                        if (entry.getIcon() != null) {
                            ...
                            drawValueWithIcon(icon, c, formatter.getPointLabel(entry), x, y, Color.WHITE, valOffset);
                        } else {
                            drawValue(c, formatter.getPointLabel(entry), x, y - valOffset, Color.WHITE);
                        }
                        //底部不覆盖x轴数值显示，减去数字size：11sp+padding：3dp
                        drawFillLine(c, x, x, y, mViewPortHandler.contentBottom()-Utils.convertDpToPixel(14), Color.WHITE);
     ...
     * @param offset
     */
    public void drawValueWithIcon(Drawable icon, Canvas c, String valueText, float x, float y, int color, float offset) {
        mValuePaint.setColor(color);
        c.drawText(valueText, x + offset, y + offset + mValuePaint.getTextSize() * 2, mValuePaint);
        drawImage(icon, c, x - offset, y + offset + +mValuePaint.getTextSize() * 2 - mValuePaint.getTextSize() / 3);
        c.drawText(valueText, x + offset, y + offset + mValuePaint.getTextSize() * 3, mValuePaint);
        drawImage(icon, c, x - offset, y + offset + mValuePaint.getTextSize() * 3 - mValuePaint.getTextSize() / 3);
    }

    private static Rect mDrawableBoundsCache = new Rect();

    public void drawImage(Drawable drawable, Canvas canvas, float x, float y) {
        ...
        drawable.setBounds(
                ...
                mDrawableBoundsCache.left + 32,
                mDrawableBoundsCache.top + 16);
