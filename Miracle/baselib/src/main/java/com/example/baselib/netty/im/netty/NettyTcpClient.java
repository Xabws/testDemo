...
import com.example.baselib.netty.im.ExecutorServiceFactory;
import com.example.baselib.netty.im.HeartbeatHandler;
 ...
 * 基于netty实现的tcp ims
 */
public class NettyTcpClient implements IClientInterface {
    ...
    private String serverUrl;// ims服务器地址组
    private IOnEventListener mOnEventListener;// 与应用层交互的listener
    private IMConnectStatusCallback mIMSConnectStatusCallback;// ims连接状态回调监听器
     ...
     */
    @Override
    public void init(String serverUrl, IOnEventListener listener, IMConnectStatusCallback callback) {
        close();
        isClosed = false;
     ...
     */
    @Override
    public void sendMsg(MsgBean msg, boolean isJoinTimeoutManager) {
    ...
    private void onConnectStatusCallback(int connectStatus) {
        ...
        switch (connectStatus) {
            ...
            case IMSConfig.CONNECT_STATE_SUCCESSFUL: {
                ...
                // 连接成功，发送握手消息
                MsgBean handshakeMsg = getHandshakeMsg();
               ...
               /* MsgBean heartbeatMsg = getHeartbeatMsg();
                ...
                System.out.println("发送心跳消息：" + heartbeatMsg + "当前心跳间隔为：" + getHeartbeatInterval() + "ms\n");
    ...
    private class ResetConnectRunnable implements Runnable {
        ...
        private int connectServer() {
            ...
            for (int j = 1; j <= IMSConfig.DEFAULT_RECONNECT_COUNT; j++) {
                ...
                try {
                    currentHost = serverUrl;// 获取host
                    currentPort = 60443;// 获取port
                    toServer();// 连接服务器
